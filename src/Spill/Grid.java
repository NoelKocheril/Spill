...
public class Grid {
	...
	private class Cell_Tuple {
		public final Cell c;
		public final int s;
		public Cell_Tuple(Cell c, int s) {
			this.c = c;
			this.s = s;
		}
		public Cell getCell() {
	...
	public void Spill (int row, int col, int strength) {
		Queue<Cell_Tuple> q = new LinkedList<Cell_Tuple>();
		Set<Cell> s = new HashSet<Cell>();
		Cell_Tuple ct = new Cell_Tuple(this.getCell(row, col),strength);
		if (ct.c.isObstacle()) return;
		q.add(ct);
		s.add(this.getCell(row, col));
		while(!q.isEmpty()) {
			Cell_Tuple _t = q.poll();
			Cell _cell = _t.getCell();
			int _row = _cell.getRow();
			int _col = _cell.getCol();
			this.setCell(_row,_col,_t.s); 
			if(_t.s <= 1) continue;
			Cell _c;
			for(int i = -1;i <= 1;i++) {
				for(int j = -1;j <= 1;j++) {
					if (this.isCell(_row+i, _col+j) && this.isClear(_row+i, _col+j)) {
						_c = this.getCell(_row+i, _col+j);
					} else continue;
					if (!s.contains(_c)) {
						s.add(_c);
						q.add(new Cell_Tuple(_c,_t.s-1));
					}
				}
			}
		}
